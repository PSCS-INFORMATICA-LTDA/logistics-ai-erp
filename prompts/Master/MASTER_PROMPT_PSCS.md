...
| Dashboard 3D | KPIs + seções recolhíveis (Frete/Transporte, Estacionamento, Lava-rápido, Participações) com filtros por placa/sócio/%. Base DEMO volumosa (4 meses diários, frete manhã/tarde, 50/50 nas vans, GHR 100% GRX) via RPC `seed_dashboard_demo` (Senha Máster). SQL: `apply-047-dashboard-demo-volume.sql` (substitui apply-046) + `reset-dashboard-demo.sql`. Sem banco de desenvolvimento separado; limpeza total = update futuro. |
