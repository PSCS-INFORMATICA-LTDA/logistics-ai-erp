...
| Dashboard 3D | Navegação por produto (Geral / Frete-Transporte / Estacionamento / Lava). Geral: pizzas de participação na **receita** e nas **despesas** por produto + KPIs. Subpastas com barras 3D; filtros placa/sócio/% no Geral e Frete. Base DEMO volumosa (4 meses diários) via RPC `seed_dashboard_demo` (Senha Máster). SQL: `apply-047-dashboard-demo-volume.sql` + `reset-dashboard-demo.sql`. |
