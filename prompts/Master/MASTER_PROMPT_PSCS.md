...
| Estacionamento / Lava | Módulos próprios: portes + tabela de preços (diária/mensal/**rotativo por hora**/lava) em **Parâmetros do pátio**; ordens em Operacional → Estacionamento / Lava-rápido; DRE **Receita Estacionamento** / **Receita Lava Rápido** no fechamento. Rotativo: **1ª hora** + **hora adicional** + **hora de saída obrigatória** + **valor estimado ao vivo**. Mensal: **vigência por aniversário** + renovação. **Lava:** status **Pronto** com aviso **WhatsApp/SMS** (“veículo pronto”); **fidelidade por placa** (parâmetro a cada **5 ou 10** lavagens → N grátis) em Parâmetros do Pátio. **Ticket:** WhatsApp + QR (`/ticket/[token]`); impressão 58/80 mm opcional. SQL: `apply-041`…`044` + `061` + `062` + `apply-063-car-wash-ready-loyalty.sql`. Na OS geral: **Tipo** e **Natureza DRE** só **Frete** e **Transporte**. |
