...
| Parâmetros | **Senha Máster - Concessão de Acessos** (frase de recuperação + permissões por tela/sócio). **Análise** = só ver; **Alteração** = criar/editar/finalizar; **Exclusão** = apagar — enforçado em CrudPage, DRE, pátio, cadastros e OS. Exclusão pede **motivo obrigatório** (modal) gravado no histórico. **Histórico de exclusões** — admin; SQL `apply-048` + `apply-049-deletion-audit-reason.sql`. |
