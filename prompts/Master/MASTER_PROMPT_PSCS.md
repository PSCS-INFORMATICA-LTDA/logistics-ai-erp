...
| Estacionamento / Lava | Módulos próprios: portes + tabela de preços (diária/mensal/**rotativo por hora**/lava) em **Parâmetros do pátio**; ordens em Operacional → Estacionamento / Lava-rápido; DRE **Receita Estacionamento** / **Receita Lava Rápido** no fechamento. Rotativo: **1ª hora** + **hora adicional** + **hora de saída obrigatória** + **valor estimado ao vivo**. Mensal: **vigência por aniversário** (`period_end_date` / `next_charge_date`) + botão **Renovar próximo período**. Diária: dias entre entrada e saída. **Ticket:** preferência **WhatsApp + QR** (link público `/ticket/[token]`, cliente apresenta no celular); impressão térmica 58/80 mm opcional. Telas op: `/operacional/estacionamento/[id]/ticket` e `/operacional/lava-rapido/[id]/ticket`. Clipe de comprovante **opcional**. SQL: `apply-041` + `apply-042` + `apply-044` + `apply-061` + `apply-062-patio-public-ticket.sql`. Na OS geral: **Tipo** e **Natureza DRE** só **Frete** e **Transporte**. |
